...
const outputJSExportsPath = path.join(
  sourcePath,
  "Script",
  "JavaScript",
  "Modules.jsc.g.h"
...
function generateJSExports(typeInfo: TypeInfo[]): string {
  const generateArgs = (parameters: ParameterInfo[]) =>
    parameters.map((paramInfo, index) => {
      if (paramInfo.type.includes("|")) {
        return undefined;
      }

      return `value_to<${paramInfo.type}>(ctx, argv[${index}])`;
    });
  const generateConstructor = (classInfo: ClassInfo) => {
    const name = isSharedPtr(classInfo)
      ? innerType(classInfo)
      : classInfo.sourceName;
    const indent =
      classInfo.ctor && classInfo.ctor.length > 0
        ? "\n                    "
        : "";
    return `std::make_unique<${name}>(${indent}${
      classInfo.ctor ? generateArgs(classInfo.ctor).join(`,${indent}`) : ""
    })${indent}.release()`;
  };
  const generateEnum = (sourceName: string, info: EnumValueInfo) =>
    `        JS_MAKE_ENUM_VALUE(${sourceName}, ${info.name}),`;
  const generateFunction = (classInfo: ClassInfo, info: FunctionInfo) => {
    const indent = info.parameters.length > 1 ? "\n                " : "";
    const funcName = info.name.toCamelCase();
    return [
      `        JS_MAKE_FUNCTION(${funcName}, {`,
      `            R_ASSERT(argc == ${info.parameters.length}, "Rainbow.${
        classInfo.name
      }.prototype.${funcName}: Expected ${info.parameters.length} argument${
        info.parameters.length === 1 ? "" : "s"
      }");`,
      `            auto self = get_instance<${classInfo.sourceName}>(object);`,
      `            ${info.returnType ? `auto ret = ` : ""}self->${
        info.name
      }(${indent}${generateArgs(info.parameters).join("," + indent)});`,
      `            return make_value(ctx, ${
        info.returnType ? "ret" : "nullptr"
      });`,
      "        }),"
    ].join(EOL);
  };
  return [
    ...
    "{",
    ...typeInfo.filter(type => type.type !== "module").map(typeInfo => {
      const prefix = typeInfo.name.toLowerCase();
      const sourceName = typeInfo.sourceName;
      return [
        ...(typeInfo.type === "class"
          ? [
              `    static const JSStaticFunction ${prefix}_static_functions[]{`,
              ...typeInfo.methods.map(info => generateFunction(typeInfo, info)),
        ...
        ...(typeInfo.type === "enum"
          ? [
              `    static const JSStaticValue ${prefix}_static_values[]{`,
              ...typeInfo.values.map(info => generateEnum(sourceName, info)),
              "        {nullptr, nullptr, nullptr, kJSPropertyAttributeNone}",
              "    };"
            ]
          : []),
        `    static const JSClassDefinition ${prefix}_definition = create_class_definition(`,
        `        "${typeInfo.name}",`,
        `        ${
          typeInfo.type === "enum" ? `${prefix}_static_values` : "nullptr"
        },`,
        `        ${
          typeInfo.type === "class" ? `${prefix}_static_functions` : "nullptr"
        },`,
        typeInfo.type === "class"
          ? [
              "        [](JSObjectRef object) {",
              isSharedPtr(typeInfo)
                ? `            ${sourceName}(static_cast<${innerType(
                    typeInfo
                  )}*>(JSObjectGetPrivate(object)));`
                : `            delete static_cast<${sourceName}*>(JSObjectGetPrivate(object));`,
              "        },"
            ].join(EOL)
          : "        nullptr,",
        typeInfo.type === "class" && typeInfo.ctor
          ? [
              ...
              "           [[maybe_unused]] const JSValueRef argv[],",
              "           JSValueRef*) {",
              `            R_ASSERT(argc == ${typeInfo.ctor.length}, "Rainbow.${
                typeInfo.name
              }: Expected ${typeInfo.ctor.length} argument${
                typeInfo.ctor.length === 1 ? "" : "s"
              }");`,
              "            JSObjectSetPrivate(",
              "                constructor,",
    ...
    ...typeInfo
      ...
      .map(
        typeInfo =>
          `    ${
            typeInfo.type === "class" ? "JS_SET_CONSTRUCTOR" : "JS_SET_ENUM"
          }(global_context, rainbow_object, ${
            typeInfo.name
          }, ${typeInfo.name.toLowerCase()});`
      ),
    "",
    "    JSObjectSetProperty(",
    "        global_context,",
...
function innerType(classInfo: ClassInfo): string {
  const match = classInfo.sourceName.match(/SharedPtr<(.*)>/);
  return !match ? classInfo.sourceName : match[1];
}

function isSharedPtr(classInfo: ClassInfo): boolean {
  return classInfo.sourceName.lastIndexOf("SharedPtr", 0) === 0;
}
