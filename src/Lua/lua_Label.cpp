...
lua_Label::lua_Label(lua_State *L) : label(nullptr)
{
	if (lua_gettop(L))
		this->label = new Label(lua_tolstring(L, 1, nullptr));
	else
...
int lua_Label::set_color(lua_State *L)
{
	...
	this->label->set_color(Colorb(r, g, b, a));
	return 0;
}
...
int lua_Label::set_font(lua_State *L)
{
	...
	this->label->set_font(font->raw_ptr());
	return 0;
}
...
{
	this->label->set_text(lua_tolstring(L, 1, nullptr));
	return 0;
}
