...
protected:
	...
	Transition(T component,
	           ...
	           TimingFunction timing)
	    : elapsed_(0), duration_(duration), delta_(delta),
	      timing_(std::move(timing)), previous_(outset), component_(component)
	{}

	float tick()
	{
		elapsed_ += rainbow::timing::kInterval;
...
public:
	Fade(T component,
	     ...
	     const int duration,
	     TimingFunction timing)
	    : Transition<T, int>(component, 0, opacity, duration, std::move(timing))
	{}
	...
	void operator()()
	{
		...
		color.a +=
		    this->timing_(0.0f, this->delta_, this->tick()) - this->previous_;
...
public:
	Move(T component,
	     ...
	     const int duration,
	     TimingFunction timing)
	    : Transition<T, Vec2f>(component, delta, duration, std::move(timing)) {}
...
{
public:
	Scale(T component, const Vec2f f, const int duration, TimingFunction timing)
