...
public:
    ...
    StableArray(size_type count) noexcept : size_(count)
    {
        ...
        auto ptr = static_cast<uint8_t*>(::operator new(bytes, std::nothrow));

        indices_ = reinterpret_cast<size_type*>(ptr);
        data_ = reinterpret_cast<value_type*>(ptr + header_size);

        R_ASSERT(reinterpret_cast<uintptr_t>(data_) % alignof(value_type) == 0,
    ...
    auto operator[](size_type i) const -> const value_type& { return at(i); }
...
private:
                ...
                F&& ordered_index)
    {
        ...
        size_type* sorted_indices = rainbow::get_temp_buffer<size_type>(count);
